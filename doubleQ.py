...
def Qlambda(w1,w2,state):
	...
	while np.linalg.norm(state-p.targ)>p.thresh:
		...
		#update w
		f=featfromstate(state)
		Qmaxopt,optact=maxQ(w1,w2,state)
		if a!=optact:
			e=np.zeros((len(f),p.A))
		ef=e[:,a]
		...
		if np.linalg.norm(next_state-p.targ)<=p.thresh:
			...
			Qmax_next, a_next=maxQmax(ww,next_state)
...
def maxQ(w1,w2,state):
	Q=[]
	for i in range(p.A):
		Q.append((Q_s_a(w1,state,i)+Q_s_a(w2,state,i)))
	Qmax=np.max(Q)
	a=np.argmax(Q)
	return Qmax,a
...
def Qlearn_main():
	state=(p.a-1)*np.random.random_sample(2)
	f=featfromstate(state)
	w1 = np.random.random_sample((len(f), p.A))
	w2 = np.random.random_sample((len(f), p.A))
	...
	for i in range(p.episodes):
		...
		if i%10==0:
			returns.append(calcret(w1,w2))
		initial_state=(p.a-1)*np.random.random_sample(2)
		w1,w2=Qlambda(w1,w2,initial_state)
		#Qmap=mapQ(w)
	return w1,w2,returns
...
if __name__=="__main__":
	...
	w1,w2,retlog=Qlearn_multirun()
	np.savez('trained_doubleQ_1000.npy.npz',w1,w2,retlog)
